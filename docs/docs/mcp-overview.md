...
- **Interoperability:** LLMs work with tools from any vendor
...
- **MCP Host:** The environment or platform where the AI model runs (e.g., Azure AI Agent). It manages the model's lifecycle and provides the compute resources needed for inference and tool execution.
...
```plaintext
┌───────────────┐   ┌─────────────┐   ┌─────────────┐
│ Azure AI      │   │ MCP Client  │   │ MCP Server  │
│ Agent         │◄─►│ (Python)    │◄─►│ (Python)    │
└───────────────┘   └─────────────┘   └─────────────┘
        │                        │
        ▼                        ▼
  Azure OpenAI           Azure Database
  Model Deployment       for PostgreSQL + pgvector
```
